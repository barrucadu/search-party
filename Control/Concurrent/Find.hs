...
module Control.Concurrent.Find
  ...
  -- * Combinators
  , (!), (?), (.!), (.?), (@!), (@?), (>!), (>?)
  , findIn, findIn', findFirstIn, findOrderedIn
  , findInMapped, findInMapped', findFirstInMapped, findOrderedInMapped
  , findBoth, findEither
  , merging
  ) where
...
--
-- This is only safe if the stream is never used again after calling
-- this function, and the order of the stream is deterministic (eg,
-- generated by 'orderedOf').
--
-- Example safe usage:
--
...
-- | Return the first non-failing result.
...
-- | Flipped infix version of 'firstFindIn'.
(.!) :: MonadConc m => [a] -> (a -> Bool) -> Find m a
(.!) = flip findFirstIn
...
-- | Flipped infix version of 'findFirstInMapped'.
(.?) :: MonadConc m => [a] -> (a -> Maybe b) -> Find m b
(.?) = flip findFirstInMapped

-- | Flipped infix version of 'findIn''.
(@!) :: MonadConc m => [a] -> (a -> Bool) -> m (Stream m a)
(@!) = flip findIn'

-- | Flipped infix version of 'findOrderedIn'.
(>!) :: MonadConc m => [a] -> (a -> Bool) -> m (Stream m a)
(>!) = flip findOrderedIn
...
-- | Flipped infix version of 'findOrderedInMapped''.
(>?) :: MonadConc m => [a] -> (a -> Maybe b) -> m (Stream m b)
(>?) = flip findOrderedInMapped
...
-- | Variant of 'findIn' which returns the first element in the list
-- matching the predicate. This is deterministic.
findFirstIn :: MonadConc m => (a -> Bool) -> [a] -> Find m a
findFirstIn f as = firstOf [if f a then success a else failure | a <- as]
...
-- | Variant of 'findIn' which returns all successes in the order in
-- which they appear in the original list. This is deterministic.
findOrderedIn :: MonadConc m => (a -> Bool) -> [a] -> m (Stream m a)
findOrderedIn f as = orderedOf [if f a then success a else failure | a <- as]
...
-- | Variant of 'findInMapped' which returns the first element in the
-- list to produce a 'Just'. This is deterministic.
findFirstInMapped :: MonadConc m => (a -> Maybe b) -> [a] -> Find m b
findFirstInMapped f = firstOf . map (maybe failure success . f)
...
-- | Variant of 'findInMapped' which returns all 'Just's in the order
-- in which they appear in the original list. This is deterministic.
findOrderedInMapped :: MonadConc m => (a -> Maybe b) -> [a] -> m (Stream m b)
findOrderedInMapped f = orderedOf . map (maybe failure success . f)
